...
```{r task 1.3}
# data
# there is no country param for everything data
...
# since we cannot mix country param with sources param, this function will do the country filter first
get_headlines <- function(sources = NULL, q = NULL, page_size = 20, page = 1, api_key = NULL){
  ...
  if(is.null(sources)){
    ...
    base.url <- "https://newsapi.org/v2/top-headlines?country=us&language=en&q="
    url <- str_c(base.url, q, "&pageSize=", page_size, "&page=", page, "&apiKey=", api_key)
            ...
            tabItem(tabName = "sources",
                    fluidRow(
                        ...
                        box(
                            ...
                            textInput("sources_key", label = "API key"),
            ...
            tabItem(tabName = "headlines",
                    fluidRow(
                        ...
                        box(
                            ...
                            width = 3,
                            solidHeader = TRUE, collapsible = TRUE,
                            textInput("headlines_key", label = "API key"),
                            checkboxGroupInput("headlines_sources", 
                                               label = "Sources",
                                               choices = c("CNN", "Fox News", "The Wall Street Journal", "Reuters")),
                            textInput("headlines_q", label = "Keywords or Phrases"),
                            numericInput("headlines_page_size", label = "Number of Results per Page", value = 20, min = 1, max = 100, step = 1),
                            numericInput("headlines_page", label = "Page Number", value = 1, min = 1, step = 1),
                            div(align="right",
            ...
            tabItem(tabName = "everything",
                    fluidRow(
                        ...
                        box(
                            ...
                            width = 3,
                            solidHeader = TRUE, collapsible = TRUE,
                            textInput("everything_key", label = "API key"),
                            ...
                            textInput("everything_q_title", label = "Keywords or Phrases in Title"),
                            textInput("everything_q", label = "Keywords or Phrases in Title and Body"),
                            numericInput("everything_page_size", label = "Number of Results per Page", value = 20, min = 1, max = 100, step = 1),
                            numericInput("everything_page", label = "Page Number", value = 1, min = 1, step = 1),
                            selectInput("everything_sort_by",
                                        label = "The Order to Sort the Articles",
                                        choices = c("relevancy", "popularity", "publishedAt")),
                            div(align="right",
                                actionButton("everything_run", label = "Run")
                            )
                        )
                    )
            )
...
server <- function(input, output) {
    ...
    data_sources <- eventReactive(input$sources_run, {
        ...
        validate(
            need(length(data_sources_reac) != 0, "No result under this condition")
        )
        data_sources_reac %>% select(name, url)
    })
    ...
    data_headlines <- eventReactive(input$headlines_run, {
        ...
        validate(
            need(length(data_headlines_reac) != 0, "No result under this condition")
        )
        data_headlines_reac %>% select(name, author, title, url)
        })
    ...
    data_everything <- eventReactive(input$everything_run, {
        ...
        validate(
            need(length(data_everything_reac) != 0, "No result under this condition")
        )
        data_everything_reac %>% select(name, author, title, url, publishedAt)
        })
